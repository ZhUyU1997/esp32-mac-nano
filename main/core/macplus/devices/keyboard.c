...
/* PCE lib optional: only used when logging unknown keys */
static const char *pce_key_to_string(pce_key_t key)
{
	(void)key;
	return NULL;
}

static mac_kbd_map_t key_map_us[] = {{PCE_KEY_BACKQUOTE, 1, {0x65}, 1, {0xe5}},
...
static mac_kbd_map_t key_map_intl[] = {{PCE_KEY_BACKQUOTE, 1, {0x65}, 1, {0xe5}},
...
static mac_kbd_map_t key_fix_keypad1[] = {{PCE_KEY_KP_7, 2, {0x79, 0x67}, 2, {0x79, 0xe7}},
...
static mac_kbd_map_t key_fix_keypad2[] = {{PCE_KEY_KP_7, 2, {0x79, 0x33}, 2, {0x79, 0xb3}},
                                          {PCE_KEY_KP_8, 2, {0x79, 0x37}, 2, {0x79, 0xb7}},
                                          {PCE_KEY_KP_9, 2, {0x79, 0x39}, 2, {0x79, 0xb9}},
                                          {PCE_KEY_KP_4, 2, {0x79, 0x2d}, 2, {0x79, 0xad}},
                                          {PCE_KEY_KP_5, 2, {0x79, 0x2f}, 2, {0x79, 0xaf}},
...
static void mac_kbd_fix_map(mac_kbd_map_t *dst, const mac_kbd_map_t *src)
{
	mac_kbd_map_t *tmp;
...
static void mac_kbd_set_data_out(mac_kbd_t *kbd, unsigned char val)
{
...
static void mac_kbd_buf_set_byte(mac_kbd_t *kbd, unsigned char val)
{
	unsigned i;
...
static void mac_kbd_set_sequence(mac_kbd_t *kbd, unsigned char *buf, unsigned cnt)
{
	unsigned i;
...
void mac_kbd_set_key(mac_kbd_t *kbd, unsigned event, pce_key_t key)
{
	...
	if (map->pcekey == PCE_KEY_NONE) {
		const char *str = pce_key_to_string(key);
		(void)str;
		return;
	}
