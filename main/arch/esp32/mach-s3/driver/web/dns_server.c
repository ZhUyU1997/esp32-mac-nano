...
struct dns_server_handle {
    bool started;
    TaskHandle_t task;
    int sock;               /* task-created UDP socket (for shutdown on stop) */
    int num_of_entries;
...
void dns_server_task(void *pvParameters)
{
    ...
    while (handle->started) {
        ...
        /* Timeout so a stopped server (started=false) can exit the
         * blocking recvfrom within ~200ms instead of hanging forever. */
        struct timeval rx_timeout = { .tv_sec = 0, .tv_usec = 200000 };
        setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &rx_timeout, sizeof(rx_timeout));

        int err = bind(sock, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
        if (err < 0) {
            ESP_LOGE(TAG, "Socket unable to bind: errno %d", errno);
            break;   /* exit; the outer code closes the socket */
        }
        ...
        while (handle->started) {
            ...
            // Timeout (no data): loop back and re-check handle->started.
            ...
            if (len < 0) {
                ...
                break;   /* outer code closes the socket (no double close) */
            }
            // Data received
            else {
        ...
        handle->sock = -1;
        handle->task = NULL;   /* tell stop_dns_server we are done */
    }
    vTaskDelete(NULL);
}
...
void stop_dns_server(dns_server_handle_t handle)
{
    if (handle) {
        ...
        /* wake the task blocked in recvfrom so it can close its socket
         * and exit; then wait for it before freeing the handle */
        if (handle->sock >= 0)
            shutdown(handle->sock, 0);
        while (handle->task != NULL)
